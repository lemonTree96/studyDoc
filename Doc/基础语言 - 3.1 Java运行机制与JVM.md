...
&emsp;&emsp; **Step 1 - 初始标记**：初始标记阶段会暂停所有其他用户线程，触发STW，并留下以GC Root为起始路径的，可以直接引用的对象。
&emsp;&emsp; **Step 2 - 并发标记**：并发标记是从GC Roots 直接关联对象开始，遍历整个对象图的过程，这个时间比较长，但不需要停顿用户线程，垃圾收集线程可以和用户线程一起并发执行。但因为有用户线程在继续运行，可能会导致已经标记过的对象状态发生变化。
&emsp;&emsp;  **Step 3 - 重新标记**：该阶段是为了修正在并发标记期间产生变化对象的标记记录 ( 因为用户线程在并发标记阶段并发运行会导致对象标记发生变化 )。重新标记阶段主要用到三色标记里的增量更新算法来进行重新标记，在重新标记过程中会暂停所有其他用户线程，触发STW，这一阶段的停顿时间 > 初始标记停顿时间，但远小于并发标记的时间。
&emsp;&emsp;  **Step 4 - 并发清理**：并发清理是指CMS对未标记的空间进行清理，在这一阶段，如果有新的对象生成，则新生成的对象不做任何处理。
&emsp;&emsp;  **Step 5 - 并发重置**：重置本次垃圾收集过程的标记数据。
&emsp;&emsp;  *CMS* 收集器的相关JVM指令如下：
&emsp; &emsp;  ● 启用CMS：`-XX:+UseConcMarkSweepGC`
&emsp; &emsp;  ● 并发的GC线程数：`-XX:ConcGCThreads`
&emsp; &emsp;  ● FullGC之后做压缩整理( 减少碎片 )：`-XX:+UseCMSCompactAtFullCollection`
&emsp; &emsp;  ● 在初始标记的时候多线程执行，缩短STW：`-XX:+CMSParallellnitialMarkEnabled`
&emsp; &emsp;  ● 在重新标记的时候多线程执行，缩短STW： `-XX:+CMSParallelRemarkEnabled`
&emsp; &emsp;  ● 在CMS GC前启动一次 MinorGC，目的是减少老年代对年轻代的引用，降低CMS 在GC的标记阶段时的开销： `-XX:+CMSScavengeBeforeRemark`

<img src="../picOfDoc/image-20230528165019439.png" alt="image-20230528165019439" style="zoom:60%;" />
...
<img src="../picOfDoc/image-20230603155622694.png" alt="image-20230603155622694" style="zoom:59%;" />

&emsp;&emsp;  与 CMS 收集器使用的内存布局不同，G1 收集器能独立管理整个GC堆 (包括新生代和老年代)，不需要与其他收集器搭配。<font color=green>G1收集器将堆内存划分为大小相同的区域 ***Region***，新生代和老年代不再是物理隔离，它们都是一部分 *Region* ( 不需要连续 )的集合</font>。<font color=red>*Region* 是G1的最小回收单元，每一块 *Region* 的大小是固定的，可以通过参数 `-XX:G1HeapRegionSize` 设置 *Region* 的大小</font>。正在使用中的 *Region* 根据其使用的角色可以分为<font color=green> ***Eden Region* ( *Eden* 区 )、 *Survivor Region* ( *Survivor* 区 )、*Old Region* ( 老年代 ) 以及 *Humongous Region* ( 大对象区 )** </font>。*Humongous Region* 是G1对巨大对象的特殊处理。当一个对象的大小超过一个阈值 ( *HotSpot* 中是分区大小的50% )，该对象就会被认为是一个巨型对象。一个巨型对象会占据一个或者多个连续 *Region*，巨型分区被认为是老年代的一部分，但是巨型对象的晋升成本很高，因此对于巨型对象，G1在年轻代收集时就会探测其存活性，一旦死亡就会在当次回收释放掉。

<img src="../picOfDoc/image-20230528221604178.png" alt="image-20230528221604178" style="zoom:60%;" />

&emsp;&emsp; 在 *Region* 分区基础上，有两个特殊的分区：**CSet ( *Collection Set* ) 和 RSet ( *Remember Set* )**。  *Region*、*CSet* 和 *RSet* 的关系如下图所示：

![image-20230603155853591](../picOfDoc/image-20230603155853591.png)

&emsp;&emsp;  ● **CSet ( *Collection Set* - 被回收的 *Region* 集合 )**
&emsp;&emsp;    *CSet* 是一组可被回收的 *Region* 的集合。<font color=green>*CSet* 中保存着每次GC中要回收的目标分区</font>。G1的分区回收是根据 *CSet* 来的，未完全死亡的分区要想被回收，首先要被放入 *CSet*，然后在 *Evacuation* 阶段，G1会针对 *CSet* 中的 *Region* 进行回收。在回收过程中，*CSet* 内的存活对象会被复制到新分配的空闲分区中。因为年轻代是整体回收，所以无论是年轻代收集还是混合收集，所有的年轻代垃圾分区都会被放入CSet。
&emsp;&emsp;  ● **RSet ( *Remember Set* - *Region* 中对象的引用来源 )**
&emsp;&emsp;     每个 *Region* 都有一个的 *RSet* 记录着引用当前分区对象的外部老年代分区对象所在的 *Region_Card* (卡标记)。<font color=green> ***RSet* 的目的是为了获取收集区域来自收集区域的引用来源，*RSet* 是一个空间换时间的数据结构**</font>。即非 *CSet* 分区对 *CSet* 分区的引用关系。这样在 *Evacuation Pauses* 阶段对 *CSet* 分区进行复制回收时，在新复制的 *Region* 中完成 *CSet* 分区存活对象的复制后，通过 *RSet* 就可以精准知道该复制对象的引用来自于哪里，从而在复制完成后可以精确的找到引用方，而不用对整个堆内存的对象进行扫描来确定引用关系。

> <font color=SlateBlue> <u>**Q1. 为什么RSet仅记录来自外部老年代分区的引用呢 ？**</u></font>
> &emsp;&emsp;    一个分区要么是 *CSet* 分区，要么不是 *CSet* 分区，所以来自本分区的引用无需记录。而无论是Young GC还是混合GC，新生代在收集后总是在 *CSet* 中的，所以来自年轻代分区的引用无需记录。

**▧ G1 收集器的收集过程**：

&emsp;&emsp;  在G1收集器中，从分代的角度开看，收集过程分为两种：<font color=red>**年轻代收集 ( Young GC )**和**混合收集周期 ( *Mixed Collection Cycle* )**</font>

![image-20230604105608592](../picOfDoc/image-20230604105608592.png)

&emsp;&emsp;   ● **年轻代收集**：当年轻代满时就会触发年轻代收集，回收年轻分区，并将存活次数达到晋升标准的对象移入老年代。年轻代收集分为以下几个步骤：
&emsp;&emsp; **Step 1 - 扫描 *GC Roots* 对象 ( *GC Roots* + *RSet* 记录的外部引用)**：年轻代收集运用可达性分析法扫描所有的年轻代分区，标记 *GC Roots* 直接关联引用链中的存活对象，除此之外还会扫描所有年轻代分区的 *RSet*，标记出老年代分区引用的对象，对 *GC Roots* 根节点进行补充。<font color=red>在标记的过程中，会导致用户线程暂停，触发 STW</font>。
&emsp;&emsp; **Step 2 - 更新 *RSet***：此阶段完成后，*RSet* 可以准确的反映老年代对所在的内存分段中对象的引用。
&emsp;&emsp; **Step 3 - 处理 *RSet***：识别被老年代对象指向的 *Eden* 中的对象，这些被指向的对象被认为是存活的对象。
&emsp;&emsp; **Step 4 - 复制对象**：标记完成后，对对象进行复制或回收，如下图所示：     

<img src="../picOfDoc/image-20230529230519009.png" alt="image-20230529230519009" style="zoom:60%;" />

&emsp;&emsp;   ● **混合收集周期**：混合收集周期内分为两个部分：**老年代并发标记 ( *Concurrent Marking* )**和**混合收集**。
&emsp;&emsp;   ① 老年代并发标记过程：
&emsp;&emsp;    随着程序的运行，越来越多的对象晋升入老年代，当老年代堆占比达到 *IHOP* 阈值(老年代占整堆比，可以通过 `-XX:InitiatingHeapOccppancyPercent` 设置，默认45%) 时就会触发老年代并发标记过程，从而确定对象存活性。G1的并发标记过程分为：初始标记 ( *Initial Mark* )，根分区扫描 ( *Root Region Scanning* )，并发标记 ( *Concurrent Mark* )，重新标记 ( *Remark* )，清除 ( *Cleanup* )。

![image-20230617175422166](../picOfDoc/image-20230617175422166.png)

&emsp;&emsp; <font color=red>**Step 1 - 初始标记 ( 扫描 *GC Roots* 对象)**</font>：初始标记负责标记出所有的 *GC Roots* 对象，这一阶段需要STW。G1收集器为了缩短用户线程停顿时间，在老年代打到 *IHOP* 阈值之后并不会马上进行初始标记，而是会等待下一次年轻代收集，利用年轻代收集的 STW 完成初始标记阶段，实现 STW 时间上的复用。
&emsp;&emsp; **Step 2 - 根分区扫描 ( *GC Roots* 对象合并 )**：年轻代收集之后，年轻代存活的对象都会被放入新的 *Survivor Region*，之后应用线程就会恢复运行。新的 *Survivor Region* 分区的对象在初始标记完成之后是存活的，对于老年代分区来说，它们和初始标记时标记出来的 *GC Roots* 对象是一致的。因此在根分区扫描阶段需要将 *Survivor Region* 内的所有对象都标记为GC Roots对象。这些存活的 *Survivor Region* 也可以被称为 *Roots Region*。此阶段不需要STW，但是这个阶段必须要在下一次年轻代收集触发之前完成，因为每次年轻代收集触发都会使得 *Survivor Region* 发生改变。

<img src="../picOfDoc/image-20230604143228905.png" alt="image-20230604143228905" style="zoom:50%;" />

&emsp;&emsp; **Step 3 - 并发标记 ( 分析 *GC Roots* 引用链 )**：并发标记阶段对 *GC Roots* 对象进行引用链分析，标记引用链上的对象为存活。此阶段不需要STW，且此阶段可能会经历年轻代回收，这意味着分析过程中对象的引用关系可能会发生改变。<font color=green>为了解决并发过程中的引用变动导致的存活对象的漏标，避免存在有效引用的对象被当作垃圾回收掉，G1通过 SATB 算法 ( 起始快照算法 *Snapshot At The Beginning* ) 来记录标记期间引用关系的改变。</font><font color=red>如果存活对象漏标，会导致对象回收时，将正在使用的对象回收，导致系统异常。</font>
&emsp;&emsp; <font color=red>**Step 4 - 重新标记( 防止存活对象漏标 )**</font>：并发标记阶段由于和用户线程一起执行，那么引用关系也会一直改变，G1在并发标记阶段使用 SATB 算法记录变化。重新标记阶段则会STW，使得引用不再变化，并解析 SATB 对对象存活性进行修正以获得完整的存活性标记。
&emsp;&emsp; **Step 5 - 清除 ( 根据收益率将待回收对象放入 *CSet* )**：G1的清除阶段并不会直接清理垃圾对象，而是会根据 *Previous Bitmap* 与 *RSet* 分析出回收收益，并根据回收收益进行排序，将回收收益率最高的 *Region* 放入 *CSet* ，等待下次年轻代收集触发时在 *Evacuation* 阶段( 根据选择的CSet来回收内存空间 )对 CSet 中的 *Region* 进行回收。这个过程不需要 STW。
&emsp;&emsp;<font color=red> **Step 6 - 存活对象复制**</font>：当G1收集器将垃圾回收后，需要将存活对象复制到新的新的内存地址上。同时由于对象地址的变化，所有指向对象旧地址的指针都要调整到对象新的地址上。<font color=red>由于在复制阶段要处理所有存活的对象，耗时会较长，因此，G1停顿时间的瓶颈主要是标记 -整理中的复制阶段 STW</font>。

> <font color=SlateBlue> <u>**Q1. G1收集器是如何计算回收收益的 ？**</u></font>
...
&emsp;&emsp;  ① 混合收集过程中，因为用户线程是和GC线程并发进行的，如果混合收集回收空间的速度跟不上用户线程分配新对象的速度。使得空闲分区越来越少，最终导致无空闲空间可用。
&emsp;&emsp;  ② 各个分区内部碎片化严重，虽然都有空闲空间，但是都达不到回收阈值成为候选分区，分区一直不被回收。那么就会导致空闲分区越来越少，最终导致分区回收时无空闲空间可用。
...
![image-20230610110700347](../picOfDoc/image-20230610110700347.png)

&emsp;&emsp;  **② 并发垃圾收集日志**：![image-20230610163010060](../picOfDoc/image-20230610163010060.png)

&emsp;&emsp;  **③ Full GC收集日志**：

<img src="../picOfDoc/image-20230610172305130.png" alt="image-20230610172305130" style="zoom:60%;" />
